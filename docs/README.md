# Nova Orchestrator: AI Task Delegation Workflow

## Description

**Nova Orchestrator** is a project designed to dynamically delegate complex queries to specialized AI agents. At the core of the system is the **Nova**, which orchestrates the problem-solving process by breaking down queries into sub-tasks and assigning each to the most suitable agent. These agents either solve sub-tasks using tools (via the **ReAct Agent**) or by reasoning step-by-step (via the **Chain of Thought Agent**). The Nova continues this iterative process until the entire task is solved, providing the final answer to the user.
...
- **Nova:** Manages the overall query-solving process by determining how to break down tasks and assigning them to appropriate agents based on the task's requirements. The user can now provide their own agents, which the Nova will consider when assigning tasks.
- **User-Created Agents:** Users can create and assign their own agents to tasks. If the Nova determines that a user-provided agent is suitable, it will utilize that agent. If not, the Nova will create its own agent.
...
3. The user can provide their own agents for the Nova to consider. If the Nova finds the user-provided agent suitable, it will assign the task to it.
...
5. Sub-tasks are solved incrementally, with results passed back to the **Nova**.
...
- **User-Created Agents:** Users can provide their own agents, which the Nova can utilize if deemed appropriate.
...
To start using **Nova Orchestrator**, follow these steps:

1. Provide a query to the **Nova**.
2. The **Nova** will create a system prompt and assign the task to either the **ReAct Agent** (if tools are required) or the **CoT Agent** (if the task relies on reasoning).
3. Users can provide their own agents, and if the Nova finds a user-provided agent suitable, it will utilize it. If not, the Nova will create its own agent.
...
- **Nova:** [Meta Prompting](https://www.promptingguide.ai/techniques/meta-prompting)

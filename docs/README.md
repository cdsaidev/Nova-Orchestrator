...
Note: Sometimes it will fail and demo might seems stuck at sometime it's actually because I limited the rate of API calls so as to prevent from hitting the rate limit.

The project employs a hierarchical and flexible design:

- **Nova:** Manages the overall query-solving process by determining how to break down tasks and assigning them to appropriate agents based on the task's requirements.
...
4. Sub-tasks are solved incrementally, with results passed back to the **Nova**.
